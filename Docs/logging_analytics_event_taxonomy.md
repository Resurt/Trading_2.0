...
| --- | --- | --- | --- |
| `market_microstructure_snapshot` | Data-only market microstructure fact: spread, mid, depth, imbalance, freshness and quality. | Implemented as `market_microstructure_snapshot`; dashboard display refresh must not write it. | `trading_date`, `micro_session_id`, `instrument_id` |
| `market_trade_sample` | Persisted data-only trade tape sample from real broker market trades. | Implemented as `market_trade_sample`; empty broker responses are status/reason only, not fake rows. | `trading_date`, `micro_session_id`, `instrument_id`, `trade_id` |
