...
`api` - СЌС‚Рѕ FastAPI BFF РґР»СЏ frontend. РћРЅ РїСЂРµРґРѕСЃС‚Р°РІР»СЏРµС‚ REST РґР»СЏ РєРѕРјР°РЅРґ, СЃРЅРёРјРєРѕРІ СЃРѕСЃС‚РѕСЏРЅРёСЏ, РєРѕРЅС„РёРіСѓСЂР°С†РёРё Рё РѕС‚С‡РµС‚РѕРІ, Р° WebSocket - РґР»СЏ live feed.

РўСЏР¶РµР»С‹Рµ РѕС‚С‡РµС‚С‹ РЅРµ СЃС‚СЂРѕСЏС‚СЃСЏ РІРЅСѓС‚СЂРё API handlers. API С‚РѕР»СЊРєРѕ СЃС‚Р°РІРёС‚ Р·Р°РґР°С‡Рё РІ `report-worker` Рё РІРѕР·РІСЂР°С‰Р°РµС‚ СЃС‚Р°С‚СѓСЃ.

Р РµР°Р»РёР·Р°С†РёСЏ С€Р°РіР° 10 РЅР°С…РѕРґРёС‚СЃСЏ РІ `apps/api/src/trading_api`. РљРѕРЅС‚СЂР°РєС‚ РѕС„РѕСЂРјР»РµРЅ С‡РµСЂРµР· Pydantic schemas, РїРѕСЌС‚РѕРјСѓ `/openapi.json` Рё `/docs` СЏРІР»СЏСЋС‚СЃСЏ РјР°С€РёРЅРЅРѕ-С‡РёС‚Р°РµРјС‹Рј РёСЃС‚РѕС‡РЅРёРєРѕРј РґР»СЏ frontend.

## Auth and control plane

API РёСЃРїРѕР»СЊР·СѓРµС‚ auth abstraction. Р’ local-dev, `historical_replay`, `sandbox` Рё `shadow`
РґРѕРїСѓСЃС‚РёРј dev provider С‡РµСЂРµР· Р·Р°РіРѕР»РѕРІРєРё `X-API-Role` Рё `X-API-Actor`. Р’ `production`
dev provider Р·Р°РїСЂРµС‰РµРЅ РЅР° startup: РЅСѓР¶РЅРѕ Р·Р°РґР°С‚СЊ `TRADING_AUTH_MODE=static_bearer`
Рё С‚РѕРєРµРЅС‹ `TRADING_API_OBSERVER_TOKEN`, `TRADING_API_OPERATOR_TOKEN` РёР»Рё
`TRADING_API_ADMIN_TOKEN` С‡РµСЂРµР· env/secret file.

Р”Р»СЏ browser WebSocket РІ production-like СЂРµР¶РёРјР°С… РёСЃРїРѕР»СЊР·СѓРµС‚СЃСЏ РєРѕСЂРѕС‚РєРѕР¶РёРІСѓС‰РёР№ ticket:
РєР»РёРµРЅС‚ РІС‹Р·С‹РІР°РµС‚ `POST /auth/ws-ticket` СЃ bearer auth, Р·Р°С‚РµРј РїРѕРґРєР»СЋС‡Р°РµС‚СЃСЏ Рє
`/ws/...?...ticket=...`. РћР±С‹С‡РЅС‹Р№ `new WebSocket()` РЅРµ РїРµСЂРµРґР°РµС‚ custom
`Authorization` header, РїРѕСЌС‚РѕРјСѓ X-API-Role-only WebSocket РґРѕСЃС‚СѓРїРµРЅ С‚РѕР»СЊРєРѕ РІРЅРµ
production.

Р Р°Р·СЂРµС€РµРЅРЅС‹Рµ СЂРѕР»Рё:

- `observer` - С‡С‚РµРЅРёРµ СЃРѕСЃС‚РѕСЏРЅРёСЏ, РѕС‚С‡РµС‚РѕРІ Рё РєРѕРЅС„РёРіСѓСЂР°С†РёРё;
- `operator` - С‡С‚РµРЅРёРµ РїР»СЋСЃ РєРѕРјР°РЅРґС‹ СѓРїСЂР°РІР»РµРЅРёСЏ Рё Р·Р°РїСѓСЃРє РѕС‚С‡РµС‚РѕРІ;
- `admin` - С‚Рµ Р¶Рµ РїСЂР°РІР°, С‡С‚Рѕ `operator`, СЃ Р±СѓРґСѓС‰РёРј СЂР°СЃС€РёСЂРµРЅРёРµРј РїРѕРґ Р°РґРјРёРЅРёСЃС‚СЂРёСЂРѕРІР°РЅРёРµ.

Р•СЃР»Рё dev-Р·Р°РіРѕР»РѕРІРѕРє РЅРµ РїРµСЂРµРґР°РЅ, СЂРѕР»СЊ СЃС‡РёС‚Р°РµС‚СЃСЏ `observer`. РљРѕРјР°РЅРґС‹ `POST /robot/start`,
`POST /robot/stop`, `POST /robot/pause`, `POST /robot/resume`,
`POST /robot/emergency-stop`, `POST /reports/daily/run` Рё `PUT /config/strategy`
С‚СЂРµР±СѓСЋС‚ `operator` РёР»Рё `admin`.

РљРѕРјР°РЅРґС‹ СѓРїСЂР°РІР»РµРЅРёСЏ РЅРµ РјРµРЅСЏСЋС‚ С‚РѕР»СЊРєРѕ in-memory state API. BFF РїРёС€РµС‚ СЃС‚СЂРѕРєСѓ
`robot_command` СЃРѕ СЃС‚Р°С‚СѓСЃРѕРј `requested` Рё audit row РІ `audit_event`. `trade-core`
С‡РёС‚Р°РµС‚ РєРѕРјР°РЅРґС‹, РїРµСЂРµРІРѕРґРёС‚ РёС… РІ `accepted/applied/rejected/failed` Рё РїСЂРёРјРµРЅСЏРµС‚
safe runtime policy Р±РµР· С„РёР·РёС‡РµСЃРєРѕРіРѕ СЂРµСЃС‚Р°СЂС‚Р° РїСЂРѕС†РµСЃСЃР°.
...
Р”Р»СЏ Р»РѕРєР°Р»СЊРЅРѕРіРѕ Vue frontend BFF СЂР°Р·СЂРµС€Р°РµС‚ CORS origins РёР· `CORS_ALLOW_ORIGINS`.
Р—РЅР°С‡РµРЅРёРµ РїРѕ СѓРјРѕР»С‡Р°РЅРёСЋ: `http://localhost:5173,http://127.0.0.1:5173`.

## Read model policy

API С‡РёС‚Р°РµС‚ РґР°РЅРЅС‹Рµ С‡РµСЂРµР· `BffReadService`, Р° РЅРµ РЅР°РїСЂСЏРјСѓСЋ РёР· РїСЂРѕРёР·РІРѕР»СЊРЅС‹С… С‚Р°Р±Р»РёС† РІ route handlers.

РћСЃРЅРѕРІРЅС‹Рµ РёСЃС‚РѕС‡РЅРёРєРё:

- `session_run` - С‚РµРєСѓС‰Р°СЏ Р±РёСЂР¶РµРІР°СЏ СЃРµСЃСЃРёСЏ Рё `micro_session_id`;
- `position_snapshot` - РїРѕСЃР»РµРґРЅРёРµ РїРѕР·РёС†РёРё;
- `broker_order` + `order_intent` - РѕС‚РєСЂС‹С‚С‹Рµ Р·Р°СЏРІРєРё Рё reason codes;
- `signal_candidate` + `blocker_event` - С‚РµРєСѓС‰РёРµ СЃРёРіРЅР°Р»С‹ Рё С„РёРЅР°Р»СЊРЅС‹Рµ blockers;
- `order_book_summary` - market overview Р±РµР· С…СЂР°РЅРµРЅРёСЏ РїРѕР»РЅРѕРіРѕ СЃС‚Р°РєР°РЅР° РЅР° РєР°Р¶РґС‹Р№ С‚РёРє;
- `hourly_report`, `daily_report`, `counterfactual_result` - РіРѕС‚РѕРІС‹Рµ РѕС‚С‡РµС‚С‹ Рё Р°РЅР°Р»РёС‚РёРєР°;
- `strategy_config` - РІРµСЂСЃРёРѕРЅРёСЂРѕРІР°РЅРЅР°СЏ РєРѕРЅС„РёРіСѓСЂР°С†РёСЏ СЃС‚СЂР°С‚РµРіРёРё.

## REST endpoints

### Market overview and readonly broker refresh

`GET /market/overview` is a cheap read-model endpoint backed by local persisted
market data and the Dashboard Live Feed cache. It returns all core-universe
instruments with explicit `quote_source`, `venue_type`, freshness, spread units,
and calibration eligibility, and it must not block on a fresh broker preflight or
unary market-data call.

Dashboard Live Feed is the readonly display path for the operator terminal. The
normal UI path is read-model first: quote-board snapshots, selected order-book
split requests and selected trade-tape split requests return local/cached data
without waiting for T-Bank `GetLastPrices`, `GetOrderBook`, `GetLastTrades`, or a
fresh dashboard preflight. Explicit refresh/fallback paths may still use bounded
readonly broker calls, but they are display-only and must not write calibration
rows or trading entities.
...
| Method | Path | РќР°Р·РЅР°С‡РµРЅРёРµ |
| --- | --- | --- |
| `POST` | `/robot/start` | Р—Р°РїСЂРѕСЃРёС‚СЊ Р·Р°РїСѓСЃРє СЂРѕР±РѕС‚Р° РІ РЅР°СЃС‚СЂРѕРµРЅРЅРѕРј СЂРµР¶РёРјРµ. |
| `POST` | `/robot/stop` | Р—Р°РїСЂРѕСЃРёС‚СЊ controlled stop. API Р±С‹СЃС‚СЂРѕ РїСЂРёРЅРёРјР°РµС‚ РєРѕРјР°РЅРґСѓ; trade-core РїСЂРёРјРµРЅСЏРµС‚ РµС‘ С‡РµСЂРµР· Р±С‹СЃС‚СЂС‹Р№ robot-command poller, runtime loop and data-only polling checks. Stop РЅРµ Р¶РґС‘С‚ РїРѕСЃС‚СЂРѕРµРЅРёСЏ РѕС‚С‡С‘С‚РѕРІ. |
| `POST` | `/robot/pause` | Р—Р°РїСЂРµС‚РёС‚СЊ РЅРѕРІС‹Рµ entries Р±РµР· РѕСЃС‚Р°РЅРѕРІРєРё РїСЂРѕС†РµСЃСЃР°. |
| `POST` | `/robot/resume` | Р’РѕР·РѕР±РЅРѕРІРёС‚СЊ РїСЂРёРµРј РЅРѕРІС‹С… entries РїРѕСЃР»Рµ pause/stop. |
| `POST` | `/robot/emergency-stop` | РќРµРјРµРґР»РµРЅРЅРѕ РїРµСЂРµРІРµСЃС‚Рё runtime РІ emergency stopped mode. |
| `GET` | `/robot/status` | РџРѕР»СѓС‡РёС‚СЊ С‚РµРєСѓС‰РµРµ СЃРѕСЃС‚РѕСЏРЅРёРµ СЂРѕР±РѕС‚Р°. |
...
| `GET` | `/dashboard/market-feed/snapshot` | Readonly live-display snapshot for the dashboard. The normal UI path is read-model first and split by purpose: quote board, selected order book, and selected trade tape do not wait for broker preflight/unary calls. Explicit combined/fallback refresh may use bounded readonly broker calls. Does not start data-only collection and does not write calibration logs. |
...
| `GET` | `/session/current` | РџРѕР»СѓС‡РёС‚СЊ С‚РµРєСѓС‰СѓСЋ Р±РёСЂР¶РµРІСѓСЋ СЃРµСЃСЃРёСЋ Рё micro-session. |
...
| `GET` | `/positions` | РџРѕР»СѓС‡РёС‚СЊ С‚РµРєСѓС‰РёРµ РїРѕР·РёС†РёРё. |
...
| `GET` | `/orders/open` | РџРѕР»СѓС‡РёС‚СЊ РѕС‚РєСЂС‹С‚С‹Рµ РѕСЂРґРµСЂР°. |
| `GET` | `/signals/current` | РџРѕР»СѓС‡РёС‚СЊ С‚РµРєСѓС‰РёРµ candidates Рё blockers. |
| `GET` | `/market/overview` | Cheap market overview read-model backed by persisted local market data and the Dashboard Live Feed cache. Returns one row per core universe instrument; full ladders remain selected-instrument only. Use `include_details=false` for quote board payloads. |
...
| `GET` | `/reports/hourly` | РџРѕР»СѓС‡РёС‚СЊ hourly reports РїРѕ С„РёР»СЊС‚СЂР°Рј. |
| `GET` | `/reports/daily` | РџРѕР»СѓС‡РёС‚СЊ daily reports РїРѕ С„РёР»СЊС‚СЂР°Рј. |
| `POST` | `/reports/daily/run` | РџРѕСЃС‚Р°РІРёС‚СЊ rebuild daily report РІ `report-worker`. |
| `GET` | `/reports/counterfactual` | РџРѕР»СѓС‡РёС‚СЊ counterfactual analytics. |
| `GET` | `/config/strategy` | РџСЂРѕС‡РёС‚Р°С‚СЊ strategy config. |
| `PUT` | `/config/strategy` | РћР±РЅРѕРІРёС‚СЊ strategy config С‡РµСЂРµР· audited change. |
...
Р РµР°Р»РёР·РѕРІР°РЅРЅС‹Рµ РїРѕР»СЏ:
...
`balance_unavailable` СЃРµР№С‡Р°СЃ РѕР¶РёРґР°РµРјС‹Р№ degraded flag: Р±Р°Р»Р°РЅСЃ РµС‰Рµ РЅРµ РїРѕРґРєР»СЋС‡РµРЅ Рє broker/account read model.
...
The API owns a small in-process `DashboardMarketFeedService` cache. The normal
operator UI path is read-model first:

- quote board: local/cached `MarketInstrumentOverview` rows. The board path does
  not wait for a fresh broker preflight or broad quote-board `GetOrderBook`
  fan-out;
- selected order book: requests with
  `include_order_book=true&include_trades=false` load only the selected
  instrument's detailed read-model row. If a fresh complete `order_book_summary`
  is already persisted, the API returns it immediately instead of blocking the
  selected card on synchronous `GetOrderBook`;
- selected trade tape: requests with
  `include_order_book=false&include_trades=true` load only the selected
  instrument's persisted/cached tape. If rows are inside the delayed display
  budget, the API returns them immediately as `persisted_data_only_trade_tape`
  instead of blocking on `GetLastTrades`;
- explicit combined/fallback refresh may use bounded readonly `GetLastPrices`,
  `GetOrderBook`, `GetLastTrades`, or `GetTradingStatus`, but it must not blank a
  fresher selected ladder or tape already available from the read model.

The full selected refresh may request a short `GetLastTrades` lookback and, when
the broker returns an empty trades list, retry once with
`DASHBOARD_TRADES_FALLBACK_LOOKBACK_MINUTES` (default 30). The selected
trade-only UI path is read-model first: if the data-only persisted tape already
has rows that are still inside the display budget, the API returns those rows
immediately as `persisted_data_only_trade_tape` instead of blocking the selected
card on a synchronous broker poll. The display budget for delayed rows is
`DASHBOARD_TRADES_DELAYED_DISPLAY_SECONDS=300`; delayed rows keep
`trade_tape_status=stale` and `trade_tape_reason=trade_exchange_ts_too_old`.
Requests with `include_order_book=false&include_trades=true` are the selected
trade-only fast path. Requests with
`include_order_book=true&include_trades=false` are the selected order-book fast
path. Both skip full preflight and broad quote-board refresh, return only the
selected instrument state, and may satisfy the request from persisted data-only
rows inside the display freshness/delayed tape window. Stale persisted tape may
be displayed as a delayed fallback within
`DASHBOARD_TRADES_DELAYED_DISPLAY_SECONDS`, but it must not masquerade as live
broker tape. These paths are display-only and must not write DB rows.
...
- `selected_details`: one `MarketInstrumentOverview` row with technical fields for
  bid/ask, mid, spread, depth, imbalance, quality components, order-book freshness
  and trade tape. The current operator UI renders only last price, spread,
  imbalance, order-book quality, source and order-book status as summary tiles;
  Bid/Ask, duplicate Mid, Depth and separate Calibration are not separate tiles;
  selected order-book arrays and selected trade rows are display-limited to 10;
...
even when the data-only collector is stopped. It is a BFF read-model backed by
Dashboard Live Feed cache first, then stored `order_book_summary`, `market_candle`
and previous-close fallback. It accepts `instruments=SBER,GAZP,LKOH,YDEX,TATN,GMKN,OZON,VTBR,T` for filtered quote
boards. The default `include_details=false` keeps payloads small: it may use cached
compact readonly order-book refreshes for the quote board, but full order-book
ladders remain selected-instrument details only.
...
It must not start any runtime stream; selected details may trigger a full-depth
readonly refresh only for the selected instrument.
...
The fallback is especially important for the selected trade-only fast path because
it allows the tape table to update with persisted data-only rows even when a
concurrent full order-book/details refresh is still running.
...
- `per_stream_status` for `order_book`, `last_price`, `candles`,
  `trading_status`, and `market_trades`. In data-only collection,
  `market_trades` is expected whenever trade collection is enabled; if it is
  absent, the dashboard can only show persisted/diagnostic fallback tape.
...
`stream_names` (`order_book`, `last_prices`, `trading_status`, `market_trades`),
`trade_collection_enabled`, `trade_sample_count`/`trade_samples_seen`,
`last_trade_sample_at`, `readonly_calls_only=true`, `real_orders_disabled=true`,
...
Endpoint РЅРµ СЃС‡РёС‚Р°РµС‚ daily report РІРЅСѓС‚СЂРё FastAPI. РћРЅ СЃС‚Р°РІРёС‚ Celery task `report_worker.rebuild_reports_for_date` С‡РµСЂРµР· Redis Рё РІРѕР·РІСЂР°С‰Р°РµС‚ job status:
...
| --- | --- |
| `/ws/dashboard` | РћР±С‰РёР№ live feed РґР»СЏ dashboard. |
...
| `/ws/reports` | РЎС‚Р°С‚СѓСЃС‹ report tasks Рё РЅРѕРІС‹Рµ reports. |

РџСЂРё РїРѕРґРєР»СЋС‡РµРЅРёРё РєР°Р¶РґС‹Р№ РєР°РЅР°Р» РѕС‚РїСЂР°РІР»СЏРµС‚ РїРµСЂРІС‹Р№ `*.snapshot`, Р·Р°С‚РµРј РїСЂРѕРґРѕР»Р¶Р°РµС‚
СЃР»Р°С‚СЊ snapshot/update СЃРѕРѕР±С‰РµРЅРёСЏ СЃ sequence РІ payload Рё heartbeat РєР°Р¶РґС‹Рµ 10
РёС‚РµСЂР°С†РёР№. РЎРѕРµРґРёРЅРµРЅРёРµ РЅРµ Р·Р°РєСЂС‹РІР°РµС‚СЃСЏ РїРѕСЃР»Рµ РїРµСЂРІРѕРіРѕ СЃРѕРѕР±С‰РµРЅРёСЏ; РїСЂРё backpressure
BFF Р·Р°РєСЂС‹РІР°РµС‚ РєР°РЅР°Р» РєРѕРґРѕРј `1011`, Р° РїСЂРё РЅРµРІР°Р»РёРґРЅРѕР№ Р°РІС‚РѕСЂРёР·Р°С†РёРё - `1008`.
...
`message_id` Рё timestamps РѕР±СЏР·Р°С‚РµР»СЊРЅС‹ РґР»СЏ deduplication Рё traceability.

РџСЂРёРјРµСЂ СЃРѕРѕР±С‰РµРЅРёСЏ `/ws/dashboard`:
...
`payload.data` СЃРѕРґРµСЂР¶РёС‚ СЃРЅРёРјРѕРє read model РЅР° РјРѕРјРµРЅС‚ РѕС‚РїСЂР°РІРєРё, Р° `payload.sequence`
РїРѕР·РІРѕР»СЏРµС‚ frontend РѕР±РЅР°СЂСѓР¶РёРІР°С‚СЊ РїСЂРѕРїСѓСЃРєРё/РїРµСЂРµРїРѕРґРєР»СЋС‡РµРЅРёСЏ.
...
`GET /session/preflight` accepts optional `instruments=SBER,GAZP,LKOH,YDEX,TATN,GMKN,OZON,VTBR,T` and
...
`OZON`, `VTBR`, `T`) must be resolved before any real/shadow risk or execution
path can treat lot/tick metadata as known. `T` is РњРљРџРђРћ В«Рў-РўРµС…РЅРѕР»РѕРіРёРёВ»
(`MOEX:T`, TQBR, ISIN `RU000A107UL4`); figi, uid, lot size and tick size are
resolver/registry facts.

`MOEX:T` commission diagnostics may expose `t_technologies_pro_free_quota`: first
15 executed buy/sell trades per Moscow trading day can be commission-free only
when Pro subscription is explicitly known active. Unknown Pro status and the 16th
or later execution use the fallback project commission. The counter is executed
trades/fills, not submitted orders and not lots.
