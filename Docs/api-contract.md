...
`GET /market/overview` is a fast local read-model endpoint. It must not block on
broker calls. It returns all core-universe instruments with explicit
`quote_source`, `venue_type`, freshness, spread units, and calibration
eligibility.
