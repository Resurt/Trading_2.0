...
| `POST` | `/market/quotes/refresh` | Explicit readonly quote/order-book refresh via T-Invest `GetLastPrices`/`GetOrderBook`; does not place or cancel orders. |
...
2. explicit readonly T-Invest quote refresh result;
...
response/read-model view, and must not call `PostOrder` or `CancelOrder`.
