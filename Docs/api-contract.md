...
- `last_price` - live mid price, если есть стакан, иначе последняя цена закрытой свечи;
- `last_price_at` - время источника цены;
- `last_price_source` - `live_order_book` или `last_candle`;
- `quote_status` - `live`, `last_close` или `unavailable`;
- `last_candle_timeframe` - таймфрейм fallback-свечи;
- spread;
- mid price;
- market quality;
- best bid/ask;
...
Полноразмерный стакан не пишется в PostgreSQL на каждый тик. Для BFF используется подготовленный агрегат `order_book_summary`. Если live стакан отсутствует или рынок закрыт, endpoint все равно должен вернуть котировку по core universe из последней доступной `market_candle`, чтобы Live Dashboard не был пустым.
