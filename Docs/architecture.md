...
| `report-worker` | Python, Celery | Часовые отчеты, дневные отчеты, rebuild jobs, counterfactual analytics, калибровочные агрегаты. |
...
| `prometheus` | Prometheus | Сбор метрик `trade-core`, `api`, `report-worker` и инфраструктуры. |
